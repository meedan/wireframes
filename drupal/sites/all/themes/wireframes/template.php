...
// Add Zen Tabs styles
if (theme_get_setting('wireframes_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'wireframes') .'/css/tabs.css');
}

function wireframes_preprocess_page(&$vars, $hook) {
...
function wireframes_preprocess_node(&$vars) {
  // Add a striping class.
  $vars['classes_array'][] = 'node-' . $vars['zebra'];

  // Merge first/last class (from wireframes_preprocess_page) into classes array of current node object.
  $node = $vars['node'];
...
function wireframes_preprocess_block(&$vars, $hook) {
  // Add a striping class.
 ...
 */
function wireframes_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('wireframes_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('wireframes_breadcrumb_home');
    if (!$show_breadcrumb_home) {
      array_shift($breadcrumb);
    }

    // Add the project taxonomy term in the $bc
    if (arg(0) == 'node' && is_numeric(arg(1))) {
      $node = node_load(arg(1));
      if (isset($node->field_project)) {
    ...
    if (!empty($breadcrumb)) {
      ...
      if (theme_get_setting('wireframes_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('wireframes_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 */	
function wireframes_id_safe($string) {
 ...
 * 
 */
function wireframes_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = wireframes_id_safe($element['#title']);
 ...
 */
function wireframes_preprocess_menu_local_task(&$variables) {
 ...
 */
function wireframes_menu_local_tasks(&$variables) {  
  $output = '';
